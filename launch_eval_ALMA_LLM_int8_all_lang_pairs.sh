...
# LANG_PAIRS=("hf://datasets/haoranxu/WMT22-Test/zh-en/test-00000-of-00001-a8c846c3e121c2f6.parquet")
...
do
    ...
    sbatch --job-name "eval-ALMA-LLM_int8-${LANG}-bz-${BZ}-SQ_LEN${SQ_LEN}" smoothquant.job --model "ALMA-7B/" --src "${SRC_LANG}" --data_path "${LANG_PAIR}" --tgt "${TGT_LANG}" --dtype "float16" --beam 5 --gen_max_tokens $SQ_LEN --batch_size $BZ --quant "LLM_int8"
