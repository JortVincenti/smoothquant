...
# LANG_PAIRS=("hf://datasets/haoranxu/WMT22-Test/en-cs/test-00000-of-00001-b92f389a2a10e4b5.parquet")
...
do
    ...
    for MODE in "${MODES[@]}"
    do
        sbatch --job-name "eval-ALMA-${LANG}-smoothquant-bit-${MODE}" smoothquant_8bit_mixed_precision.job --model "ALMA-7B/" --src "${SRC_LANG}" --data_path "${LANG_PAIR}" --tgt "${TGT_LANG}" --dtype "float16" --beam 5 --quant "smooth_8bit" --mode $MODE
    done
done