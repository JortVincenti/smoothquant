...
do
    ...
    for MODE in "${MODES[@]}"
    do
        sbatch --job-name "eval-ALMA-${LANG}-smoothquant-bit-${MODE}" smoothquant_8bit.job --model "ALMA-7B/" --src "${SRC_LANG}" --data_path "${LANG_PAIR}" --tgt "${TGT_LANG}" --dtype "float16" --beam 5 --quant "smooth_8bit" --mode $MODE
    done
done